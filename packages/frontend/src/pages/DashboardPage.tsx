...
export function DashboardPage() {
  ...
  const [aiSpec, setAiSpec] = useGlobalSpec<BaseViewSpec>(appId, 'dashboard');
  ...
  const allColumns = useMemo(() => {
    ...
    // Display name: spec.valueLabels[groupField.key][value] → alias, else raw value
    const vl = aiSpec?.valueLabels?.[groupField.key] ?? {};
    return [...enumCols, ...extraCols].map(({ value, items: colItems }) => ({
      value,
      displayName: vl[value] ?? value,
      items: colItems,
    }));
  }, [baseItems, groupField, aiSpec]);
  ...
  function startRename(value: string, displayName: string) { setEditingCol(value); setEditValue(displayName); }

  function commitRename(value: string) {
    const trimmed = editValue.trim();
    setEditingCol(null);
    if (!trimmed || !groupField) return;

    // Merge the new label into spec.valueLabels — persisted via setAiSpec
    const updatedVL = {
      ...(aiSpec?.valueLabels ?? {}),
      [groupField.key]: {
        ...(aiSpec?.valueLabels?.[groupField.key] ?? {}),
        [value]: trimmed,
      },
    };

    if (aiSpec) {
      // Existing spec: just patch valueLabels
      setAiSpec({ ...aiSpec, valueLabels: updatedVL });
    } else {
      // No spec yet: create a minimal one so the rename survives reload
      setAiSpec({
        version: '1.0',
        name: 'Custom column names',
        layout: 'kanban',
        fields: (vocabulary?.fields ?? []).map((f) => ({ key: f.key, visible: true })),
        groupBy: groupField.key,
        filters: [],
        limit: 100,
        valueLabels: updatedVL,
      });
    }
  }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-shrink-0 bg-white border-b border-gray-100 px-5 py-2.5 flex items-center gap-3">
          ...
          <button onClick={() => setAiSpec(null)} className="flex items-center gap-1 text-xs text-gray-500 hover:text-red-600 border border-gray-200 rounded-lg px-2.5 py-1.5 transition-colors">
            <RotateCcw size={11} /> Reset AI view
          </button>
        </div>
    ...
    <div className="flex h-full overflow-hidden bg-gray-50">
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 bg-white border-b border-gray-100 px-5 py-2.5 flex items-center gap-3 flex-wrap">
          ...
          {hiddenList.map((c) => (
            <button key={c.value} onClick={() => showColumn(c.value)}
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="text-xs text-gray-400">{baseItems.length} items · {visibleColumns.length} columns</span>
            {aiSpec && (
              <button onClick={() => setAiSpec(null)} className="flex items-center gap-1 text-xs text-gray-500 hover:text-red-600 border border-gray-200 rounded-lg px-2.5 py-1.5 transition-colors">
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden">
          <div className="flex h-full p-4 gap-3" style={{ minWidth: 'max-content' }}>
            {!groupField ? (
              <div className="flex items-center justify-center w-full text-sm text-gray-400 px-8">No groupable field in this domain's vocabulary.</div>
            ) : visibleColumns.length === 0 ? (
              <div className="flex items-center justify-center w-full text-sm text-gray-400">All columns hidden — ask AI to show them.</div>
            ) : (
              visibleColumns.map((col) => (
                <KanbanColumn key={col.value} col={col} vocabulary={vocabulary}
                  isEditing={editingCol === col.value} editValue={editValue}
                  onHide={() => hideColumn(col.value)}
                  onStartRename={() => startRename(col.value, col.displayName)}
                  onEditChange={setEditValue} onEditCommit={() => commitRename(col.value)} />
              ))
            )}
          </div>
        </div>
...
function KanbanColumn({ col, vocabulary, isEditing, editValue, onHide, onStartRename, onEditChange, onEditCommit }: {
  col: ColumnDef; vocabulary: AppVocabulary | null;
  ...
  return (
    <div className={`w-[272px] flex-shrink-0 flex flex-col rounded-xl bg-white border border-gray-200 border-t-[3px] ${accent} overflow-hidden h-full`}>
      <div className="flex items-center justify-between px-3 py-2.5 bg-gray-50/80 flex-shrink-0">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {isEditing ? (
            <input autoFocus value={editValue} onChange={(e) => onEditChange(e.target.value)}
              onBlur={onEditCommit} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === 'Escape') onEditCommit(); }}
            ...
            <button onDoubleClick={onStartRename} title="Double-click to rename"
        ...
        <button onClick={onHide} title="Hide column"
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2">
        {col.items.length === 0
          ...
          : col.items.map((item) => <ItemCard key={item.id as string} item={item} vocabulary={vocabulary} />)
        }
      </div>
    </div>
  );
...
function ItemCard({ item, vocabulary }: { item: Item; vocabulary: AppVocabulary | null }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-3 hover:border-indigo-200 hover:shadow-sm transition-all cursor-pointer">
      <p className="text-[13px] font-medium text-gray-900 leading-snug line-clamp-2">{title}</p>
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1.5">
          {tags.map((tag) => <span key={tag} className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">{tag}</span>)}
        </div>
      )}
      <div className="flex items-center justify-between mt-2">
        <div className="flex items-center gap-1.5">
          {priorityCfg && <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border capitalize ${priorityCfg.color}`}>{priorityCfg.label}</span>}
          {amount != null && <span className="text-[10px] text-emerald-600 font-medium">${Number(amount).toLocaleString()}</span>}
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-mono text-gray-300">#{idStr}</span>
          {assignee && (
            <div className="w-5 h-5 rounded-full bg-indigo-100 flex items-center justify-center text-[9px] font-bold text-indigo-600" title={assignee}>
